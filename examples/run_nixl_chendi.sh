RANK=0 VLLM_SKIP_WARMUP=true PT_HPU_LAZY_MODE=1 UCX_TLS=tcp VLLM_NIXL_SIDE_CHANNEL_HOST=localhost VLLM_NIXL_SIDE_CHANNEL_PORT=6559 vllm serve Qwen/Qwen3-0.6B --gpu-memory-utilization 0.3 --host localhost --disable-log-stats --port 8100 --max-model-len 8192 --seed 42 --kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_both","kv_buffer_device":"cpu"}' 2>&1 | tee nixl/nixl_server1.log &
pid_server1=$(($!-1))
...
python3 ../vllm-gaudi/tests/v1/kv_connector/nixl_integration/toy_proxy_server.py --prefiller-host localhost --prefiller-port 8100   --decoder-host localhost --decoder-port 8200   --host=localhost --port 18192 2>&1 | tee nixl/nixl_proxy.log &
